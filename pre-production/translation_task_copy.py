...
all_en_ner = [n for ex in train_data for n in ex['en_ner']]  
...
class Encoder(nn.Module):
    ...
    def forward(self, token, pos, ner):
        ...
        hidden = torch.tanh(self.fc_hidden(torch.cat((hidden[0:1], hidden[1:2]), dim=2)))  
        ...
        for batch in loader:
            ...
            for i in range(src_token.shape[0]):
                ...
                try:
                    ...
                    pass
                hyp_tokens = [idx2word.get(tok, '<unk>') for tok in hyp_ids[1:]]  
                hyp = ' '.join(hyp_tokens)
