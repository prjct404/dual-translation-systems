...
# Use larger subsets for better training
train_data = train_data.select(range(50000))  # Increased from 10k
val_data = val_data.select(range(1000))  # Increased from 500
test_data = test_data.select(range(1000))  # Increased from 500
...
train_loader = DataLoader(train_ds, batch_size=32, shuffle=True, collate_fn=collate_fn, num_workers=4)  # Reduced batch size to 32 for memory
...
class Encoder(nn.Module):
    ...
    def forward(self, token, pos, ner):
        ...
        hidden = torch.tanh(self.fc_hidden(torch.cat((hidden[0:1], hidden[1:2]), dim=2)))  # For multi-layer, adjust accordingly; here for num_layers=1 bidir
        cell = torch.tanh(self.fc_cell(torch.cat((cell[0:1], cell[1:2]), dim=2)))
        for layer in range(1, self.lstm.num_layers):
            h = torch.tanh(self.fc_hidden(torch.cat((hidden[2*layer:2*layer+1], hidden[2*layer+1:2*layer+2]), dim=2)))
            c = torch.tanh(self.fc_cell(torch.cat((cell[2*layer:2*layer+1], cell[2*layer+1:2*layer+2]), dim=2)))
            hidden = torch.cat((hidden, h), dim=0)
            cell = torch.cat((cell, c), dim=0)
